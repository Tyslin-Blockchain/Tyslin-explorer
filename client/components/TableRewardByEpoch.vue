<template>
    ...
    <section v-else>
        ...
        <div
            ...
            <p class="tyslin-empty__description">No reward found</p>
        </div>

        <p
            v-if="total > 0"
            class="tyslin-total-items">{{ _nFormatNumber('reward', 'rewards', total, realTotal) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="tyslin-table--reward tyslin-table--reward-by-epoch">
            <template
                slot="address"
                slot-scope="props">
                <nuxt-link
