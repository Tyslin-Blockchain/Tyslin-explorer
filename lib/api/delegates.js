...
module.exports = function (app) {
    function Active () {
        this.getActive = function (cb) {
            request.get({
                url : app.get('lisk address') + '/api/delegates/?orderBy=rate:asc&limit=11',
                json : true
    ...
    this.getStandby = function (n, error, success) {
        ...
        request.get({
            ...
            if (err || response.statusCode !== 200) {
                ...
                body.delegates  = parseDelegates(body.delegates);
                body.totalCount = (body.totalCount - 11);
    ...
    this.getNextForgers = function (error, success) {
        request.get({
            url : app.get('lisk address') + '/api/delegates/getNextForgers?limit=11',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
