<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="tyslin-empty">
            <i class="fa fa-binoculars tyslin-empty__icon"/>
            <p class="tyslin-empty__description">No event found</p>
        </div>

        <p
            v-if="total > 0"
            class="tyslin-total-items">{{ _nFormatNumber('item', 'items', total, realTotal) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="tyslin-table--events">
